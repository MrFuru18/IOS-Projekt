        ...
        DispatchQueue.global().async {
            ...
            DispatchQueue.main.async {
                if(self.success) {
                    ...
                    var imgulr = "https://openweathermap.org/img/wn/\(String(describing: self.weatherData?.weather?[0].icon))@4x.png".replacingOccurrences(of: "Optional(\"", with: "").replacingOccurrences(of: "\")", with: "")
                    self.imageView.loadFrom(URLAddress: imgulr)
                    print(imgulr)
                }
                else
                {
